...
function ProductDetailCard() {
  ...
  let design = pathname.substring(pathname.lastIndexOf("/") + 1);
  design = "1003";
  ...
  let product = findObjectWithKey(products, "design", design);
  getVariants = (product) => {
    let variantArray = [];
    product.files.forEach((item) => {
      if (item.variant !== "") {
        variantArray.push(item.variant);
      }
    });
  };
  let productVariants = pro;
