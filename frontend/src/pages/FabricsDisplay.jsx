...
import classes from "./FabricsDisplay.module.css";
...
function FabricDisplay() {
  ...
  const [selectedOrigin, setSelectedOrigin] = useState({'United States':false,'Italy':false});
  const [selectedEndUse, setSelectedEndUse] = useState({drapery:false,upholstery:false,multipurpose:false});
  const [allStates,setAllStates] = useState({selectedEndUse,selectedOrigin})
  ...
  return (
    <>
      <div className={classes.bread}>
        Home <span style={{ fontSize: "10px" }}>{">"}</span> Fabric
      </div>
